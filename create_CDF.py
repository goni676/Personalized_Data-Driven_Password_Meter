    ...
    with open(file, "r") as file:
        for line in file:
            ...
            if line:
                line = line.strip()
                if title != "particular":
                    tmp = PESrank.main(line, os.getcwd())
                    data.append(tmp)
                else:
                    tmp = PESrank_new.main(line, os.getcwd(), country, top_values)
                    data.append(tmp)
    data.sort()
    return data
...
def create_CDF(test_file, country):
    ...
    for n in ["100", "500", "1000", "5000"]:
        ...
        plt.title(f'Spain (Top-{n})')
        plt.savefig(f'spain_{n}_trygskjjsl.png', dpi=300)
